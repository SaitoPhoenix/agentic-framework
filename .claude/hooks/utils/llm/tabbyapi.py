...
def prompt_llm(prompt_text):
    """
    Base TabbyAPI LLM prompting method using LOCAL_MODEL.
    ...
    try:
        ...
        # TabbyAPI uses OpenAI-compatible API
        base_url = os.getenv("LOCAL_LLM", "http://localhost:5000")
        model = os.getenv("LOCAL_MODEL", "Dolphin-Mistral-24B-Venice-Edition_EXL3_4.0bpw")
        
        # TabbyAPI uses the TABBYAPI_APIKEY from env
        api_key = os.getenv("TABBYAPI_APIKEY", "")
        
        client = OpenAI(
            base_url=f"{base_url}/v1",
            api_key=api_key if api_key else "dummy",  # Some servers need dummy key
        )
...
def generate_completion_message():
    """
    Generate a completion message using TabbyAPI LLM.
...
def generate_agent_name():
    """
    Generate a one-word agent name using TabbyAPI.
...
Name:"""
    ...
    try:
        ...
        if response:
            ...
            raise Exception("No response from TabbyAPI")
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--completion":
            ...
            if response:
                ...
                print("Error calling TabbyAPI")
    else:
        print(
            "Usage: ./tabbyapi.py 'your prompt here' or ./tabbyapi.py --completion or ./tabbyapi.py --agent-name"
        )
