...
def check_tool_permission(tool_identifier: str, worktree_config: Dict[str, Any], 
                         global_config: Dict[str, Any]) -> Tuple[str, str]:
    """
    ...
    for pattern, permission in worktree_permissions.items():
        if matches_pattern(tool_identifier, pattern):
            ...
            return perm, f"Worktree pattern permission for '{tool_identifier}' (matched '{pattern}'): {perm}"
...
def matches_pattern(tool_identifier: str, pattern: str) -> bool:
    ...
    if tool_identifier.startswith('Bash(') and pattern.startswith('Bash('):
        ...
        tool_cmd = tool_identifier[5:-3] if tool_identifier.endswith(':*)') else tool_identifier[5:-1]
...
def match_worktree_patterns(worktree_name: str, patterns: list) -> Optional[Dict[str, Any]]:
    """
...
def get_worktree_permissions(worktree_name: str, 
                           permissions_config: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """
