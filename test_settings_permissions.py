...
from pre_tool_use import load_settings_permissions, check_tool_permission, format_tool_identifier, matches_pattern
...
def test_pattern_matching():
    ...
    for tool_identifier, pattern, expected in test_cases:
        ...
        assert result == expected, f"Pattern matching failed for {tool_identifier} vs {pattern}"
...
def test_permission_checking():
    ...
    for tool_name, tool_input, expected_decision, expected_source in test_cases:
        ...
        worktree_config = {}
        global_config = {"default_permission": "ask", "always_allow": [], "always_deny": []}
        
        decision, reason = check_tool_permission(tool_identifier, worktree_config, global_config)
        ...
        if expected_source == "settings.json":
            assert "settings.json" in reason, f"Expected settings.json in reason, got: {reason}"
        elif expected_source == "default":
            assert "default" in reason.lower(), f"Expected default in reason, got: {reason}"
